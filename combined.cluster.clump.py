...
class Parser():
    ...
    def __init__(self, inputfile,lengthfile,af,ncutoff,normalize):
        ...
        for key in self.db:
            ...
            #print 'Medoids' + '\t' + str(self.medoids[key])
            protID = key[1]
    ...
    def __call__(self):
        ...
        for record in self.clumps:
            print '\t'.join(record) + '\t' + str(self.clumps[record]) + '\t' + str(self.medoids[record])
...
def main():
    ...
    if(opts.controlperms and opts.permutations):
        ...
        if opts.titles:
            print "GENE\tPROTEIN_ID\tSTUDY_NAME\tRaw_Clump_Score\tMedoids"
        MyParser()
